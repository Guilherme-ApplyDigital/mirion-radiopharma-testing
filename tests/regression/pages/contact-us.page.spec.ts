...
const auditedPage = AUDITED_PAGES.find((page) => page.key === 'contact-us');

if (!auditedPage) {
  throw new Error('Missing route config for contact-us');
}

test(
  'should load contact us page with correct title and heading '
    + '@allure.label.epic:Regression '
    + '@allure.label.feature:Pages '
    + '@allure.label.story:contact-us '
