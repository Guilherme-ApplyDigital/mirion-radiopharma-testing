...
const auditedPage = AUDITED_PAGES.find((page) => page.key === 'home');

if (!auditedPage) {
  throw new Error('Missing route config for home');
}

test(
  'should load home page with correct title and heading '
    + '@allure.label.epic:Regression '
    + '@allure.label.feature:Pages '
    + '@allure.label.story:home '
